 ...
 */
package com.javen.orm.persist.json;
...
import com.javen.orm.annotation.json.Key;
import com.javen.orm.inner.ClassSpecRegistry;
import com.javen.orm.inner.ConverterRegistry;
import com.javen.orm.inner.PersistUtils;
...
import com.javen.orm.inner.ReflectionUtils;
...
public class JSONSerializer<ModelType extends Model> {
	...
	public ModelType deserialize(JSONObject obj) throws JSONException {
		ModelType model = ReflectionUtils.newInstance(cls);
		FieldSpec<KeyAnn>[] keySpecs = ClassSpecRegistry.getJsonKeySpecs(cls);
		for (FieldSpec<KeyAnn> spec : keySpecs) {
	...
	private void readFromModelAndPutToJSON(ModelType item,
		...
		if (keyParts != null) {
			...
			Object columnVal = ReflectionUtils.getFieldVal(item, spec.field);
			try {
	...
	private void readFromJSONAndSetFieldVal(ModelType model,
		...
		if (keyParts != null) {
			...
			try {
				...
				if (!NULL.equals(val)) {
					ReflectionUtils.setFieldVal(model, spec.field, val);
				} else {
