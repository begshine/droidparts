...
import static com.javen.orm.util.Strings.join;
...
import com.javen.orm.contract.SQL;
...
import com.javen.orm.persist.sql.AbstractEntityManager;
...
public final class PersistUtils implements SQL.DDL {
	...
	public static String getSQLCreate(String tableName,
		...
		for (FieldSpec<ColumnAnn> spec : specs) {
			if (DB.Column.ID.equals(spec.ann.name)) {
				// already got it
				continue;
			}
			sb.append(SEPARATOR);
			...
			if (TypeHelper.isEntity(spec.field.getType())) {
	...
	private static void appendForeignKeyDef(FieldSpec<ColumnAnn> spec,
		...
		Class<? extends Entity> entityType = spec.field.getType().asSubclass(
				Entity.class);
		String foreignTableName = ClassSpecRegistry.getTableName(entityType);
		sb.append("FOREIGN KEY(");
		sb.append(spec.ann.name);
		sb.append(") REFERENCES ");
		sb.append(foreignTableName);
		sb.append("(").append(DB.Column.ID).append(") ON DELETE CASCADE");
	}
